...
int main()
{
	...
	char buf[5] = { 0x61, 0x62, 0x63, 0x01, 0x32 };
...
#if 1 //0x0A 是换行还是文件结束符
	FILE *fp = fopen(TESTFILE3, "wb");
	char buf[] = {0x61, 0xff, 0x62, 0x63, 0x01, 0x32, 0x0a, 0x66, 0xff, 0x62, 0x64, 0x0a, 0x67, 0x68};
	fwrite(buf, sizeof(buf[0]), sizeof(buf)/sizeof(buf[0]), fp);
	fclose(fp);
	fp = fopen(TESTFILE3, "r");
	char c;
#if 1 //文本文件可以通过EOF来判断，EOF=0xff=-1, 不在0-127  Ascci码范围，所以可以使用EOF， 但是二进制则不可以
