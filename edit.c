...
void bi_ji(FILE * fp, char fname[100])
{
	...
	do {			//编辑操作菜单
		...
		case 1:
			...
			break;	//字符串查找
		case 2:
			head = str_ins(head, fname);
			break;	//字符串插入
		case 3:
			head = str_del(head, fname);
			break;	//字符串删除
		case 4:
			head = str_rep(head, fname);
			break;	//字符串替换
		}
...
Find_data str_fnd_xh(Sjjd head, char *str)
{
	...
	do {
		...
		if (!strcmp(p->Data, str)) {
			...
			p_fd->sjjd_next = p->next, p_fd->sjjd_last = p->last, p_fd->num = cnt, p_fd->jd_num =
			    toal_jd, p_fd->fd_next = NULL;
			if (head_fd == NULL)
				head_fd = tail_fd = p_fd;
			else
				tail_fd->fd_next = p_fd;
			tail_fd = p_fd;
			flag = 1;
		}
		if (p->next == NULL)
...
Sjjd str_del(Sjjd head, char *fname)
{
	...
	if (head_fd == NULL)	//要删除的字符串不存在处理
		goto end;
	printf
	    ("请选择删除对象（“0”表示删除全部查找到的对象，选择删除时每次只能删除一个！）：\n");
      re:scanf("%d", &choice);
	while ((tmp = getchar()) != '\n' && tmp != EOF) ;	//清除输入缓冲区
...
Sjjd str_rep(Sjjd head, char *fname)
{
	...
	if (head_fd == NULL)	//待替换字符不存在的处理
		goto end;
	printf
	    ("请选择替换对象（“0”表示替换全部查找到的对象，选择替换时每次只能替换一个！）：\n");
      re:scanf("%d", &choice);
	while ((tmp = getchar()) != '\n' && tmp != EOF) ;	//清除输入缓冲区
	if (choice < 0)		//限定输入的对象只能为>=0的数
	{
...
void qut_y_save(Sjjd head, FILE * fp, char *fname)
{
	...
	fclose(fp);
	fp = fopen(fname, "w");	//改变文件打开属性，切换为写操作
	//将链表内容输入到文件中
	do {
